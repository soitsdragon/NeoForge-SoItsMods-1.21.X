	...
	public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		...
		if (!master.equals("soits_plushies")) {
			if (!SoitsPlushiesModPlayerAnimationAPI.animations.isEmpty())
				SoitsPlushiesModPlayerAnimationAPI.animations.clear();
			return;
		}
		PlayerModel<T> model = (PlayerModel<T>) (Object) this;
		Player player = null;
		...
		if (data.getBoolean("ResetPlayerAnimation")) {
			...
			SoitsPlushiesModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		SoitsPlushiesModPlayerAnimationAPI.PlayerAnimation animation = SoitsPlushiesModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = SoitsPlushiesModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				SoitsPlushiesMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			SoitsPlushiesModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, SoitsPlushiesModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			SoitsPlushiesModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
